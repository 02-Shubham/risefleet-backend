...
export function startWsServer(httpServer) {
    ...
    trackerEvents.on('tracker:live', (data) => broadcast('tracker:live', data));
    trackerEvents.on('tracker:unknown', (data) => broadcastAdmins('tracker:unknown', data));
}
async function authenticate(ws, req) {
    ...
    try {
        ...
        if (decoded.role === 'Customer') {
            ...
            devices.forEach((d) => imeis.add(d.imei));
        }
